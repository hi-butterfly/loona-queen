...
$.getJSON('item.min.json', function(items){
    $.each(items, function(i, item){
        if (i<6) {
            $("#coin1>tbody").append(`<tr><td><div><h4 class="ui image header"> <img src="img/0${item.img}.info.iconRaw.png" class="ui mini rounded image"><div class="content"> ${item.name}<div class="sub header">${isChange(item.num)}</div></h4></div></td><td>${item.cnt}</td><td>${item.price}</td><td><div class="ui action input"> <input type="number" placeholder="0" style="width:100px" min="0"> <button class="ui green button cntMax" onclick="$('#coin1 tr td input').eq(${i}).val(${item.max})"> 최대</button></div></td></tr>`);
        } else if (i<19) {
            $("#coin2>tbody").append(`<tr><td><h4 class="ui image header"> <img src="img/0${item.img}.info.iconRaw.png" class="ui mini rounded image"><div class="content"> ${item.name}<div class="sub header">교환 불가</div></h4></td><td>${item.cnt}</td><td>${item.price}</td><td><div class="ui action input"> <input type="number" placeholder="0" style="width:100px" min="0"> <button class="ui green button cntMax" onclick="$('#coin2 tr td input').eq(${i-6}).val(${item.cnt})"> 최대</button></div></td></tr>`);
        } else {
            $("#coin3>tbody").append(`<tr><td><h4 class="ui image header"> <img src="img/0${item.img}.info.iconRaw.png" class="ui mini rounded image"><div class="content"> ${item.name}<div class="sub header">교환 불가</div></h4></td><td>${item.cnt}</td><td>${item.price}</td><td><div class="ui action input"> <input type="number" placeholder="0" style="width:100px" min="0"> <button class="ui green button cntMax" onclick="$('#coin3 tr td input').eq(${i-19}).val(${item.max})"> 최대</button></div></td></tr>`);   
        }
    });
});